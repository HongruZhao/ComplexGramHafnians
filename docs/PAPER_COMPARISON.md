...
| `X ∈ ℂ^{k×2n}`, iid `CN(0,1)` | `ComplexColumnMatrix n k`; `circularGaussianColumnMatrixMeasure n k` | A tuple of `2n` independent Gaussian columns, each in `ℂ^k` |
| `haf(XᵀX)` | `gramHafnianObservable n k X` | `rowMatrix` converts the column tuple to the literal rectangular matrix; the Gram product has no conjugation |
| `σ_{k,n}` | `gramHafnianSigma k n` | Square root of the proved absolute second moment |
| `B_{k,n}` | `CurrentPRL.paperBkn k n` | Exact finite product coefficient; argument order is `k,n` |
| `b_n` | `CurrentPRL.paperBn n` | `2 Γ(n+1/2)/(√π Γ(n))` |
...
| `haf(S)` | `fullSymmetricHafnian p` | Hafnian of the assembled symmetric matrix; equal to the edge hafnian for every diagonal |
| `σ_n` | `SymmetricGaussianHafnian.sigma n` | `√((2n−1)!!)` |
...
with variance `1/2`. No inverse-moment, small-ball, or density assumption is
passed to either public theorem. The sufficient conditions `n ≥ 1`, `k ≥ 4n`
...
| `E\|haf(XᵀX)\|² = (2n−1)!! ∏(k+2q)` | `Theorem21.exactSecondMoment` | `actualGramFirstMomentReal_eq_closedFirstMoment`, with the literal integral and double-factorial translation |
| The same formula for `σ²` | `Theorem21.rmsSquared` | `gramHafnianSigma_sq` |
...
| Existence and explicit bound for `θ`, `k ≥ 8n` | `Theorem21.logarithmicRemainder` | The exact log identity, remainder envelope, and large-dimension envelope inequality |
...
| `b_n ≤ 2√(n/π)` | `Theorem23.elementaryCoefficient` | `paperBn_le_two_div_sqrt_pi_mul_sqrt` with exact square-root algebra |
